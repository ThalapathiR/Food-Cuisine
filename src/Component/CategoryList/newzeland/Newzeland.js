...
import newzwland from "../newzeland/newzwland.json";
...
export const Newzeland = () => {
  ...
  useEffect(() => {
    try {
      setCuisines(newzwland.new_zealand_cuisines);
      setLoading(false);
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!newCuisine.strImage) errors.strImage = "Meal image is required.";
    return errors;
  };
