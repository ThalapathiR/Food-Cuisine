...
import russia_Cuisinedd from '../russia/russiaCuisine.json';
import 'bootstrap/dist/css/bootstrap.min.css';
import { Modal, Button, Card, Form } from 'react-bootstrap';

export const CusineRussia = () => {
  ...
  useEffect(() => {
    try {
      setCuisines(russia_Cuisinedd.russia_Cuisine);
      setLoading(false);
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!newCuisine.rating || newCuisine.rating < 1 || newCuisine.rating > 5)
      errors.rating = 'Rating must be between 1 and 5';
    if (!newCuisine.strImage) errors.strImage = 'Image is required';
    if (!newCuisine.iconic_place) errors.iconic_place = 'Iconic place is required';
    if (!newCuisine.description) errors.description = 'Description is required';
    if (!newCuisine.ingredients) errors.ingredients = 'Ingredients are required';
    return errors;
  };
  ...
  return (
    <div className="india_cusi">
      <div className="search">
        ...
        />
        <Button variant="primary" className="add-meal-btn" onClick={() => setShowForm(true)}>
          <span>Add Meal</span>
        </Button>
      </div>
      ...
      <div className="cuisine-grid">
        {filteredCuisines.map((cuisine, index) => (
          <Card key={index} className="cuisine-item" style={{ width: '18rem', cursor: 'pointer' }} onClick={() => handleImageClick(cuisine)}>
            <Card.Img variant="top" src={cuisine.strImage} width="200px" height="200px" alt={cuisine.strMeal} />
            <Card.Body>
              <Card.Text>
                <h3>{cuisine.name}</h3>
              </Card.Text>
          ...
          <Modal.Body>
            <Form onSubmit={handleSubmit}>
              ...
              <Form.Group className="mb-3" controlId="mealRating">
                ...
                />
                <Form.Control.Feedback type="invalid">{formErrors.rating}</Form.Control.Feedback>
              </Form.Group>
              ...
              <Form.Group className="mb-3" controlId="iconicPlace">
                ...
                />
                <Form.Control.Feedback type="invalid">{formErrors.iconic_place}</Form.Control.Feedback>
              </Form.Group>
              ...
              <Form.Group className="mb-3" controlId="mealIngredients">
                ...
                />
                <Form.Control.Feedback type="invalid">{formErrors.ingredients}</Form.Control.Feedback>
              </Form.Group>
              ...
              <Form.Group className="mb-3" controlId="mealImage">
                ...
                />
                {newCuisine.strImage && (
                  <img src={newCuisine.strImage} alt="Preview" style={{ width: '100px', marginTop: '10px' }} />
                )}
                <Form.Control.Feedback type="invalid">{formErrors.strImage}</Form.Control.Feedback>
              </Form.Group>
